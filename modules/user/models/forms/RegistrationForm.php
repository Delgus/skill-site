...
class RegistrationForm extends Model
{
	...
	public function rules()
	{
		$rules = [
			...
			['password', 'match', 'pattern' => Yii::$app->getModule('user')->passwordRegexp],
	...
	{
		return [
			'username'        => Yii::$app->getModule('user')->useEmailAsLogin ? 'E-mail' : UserManagementModule::t('front', 'Login'),
	...
	{
		return Yii::$app->mailer->compose(Yii::$app->getModule('user')->mailerOptions['registrationFormViewFile'], ['user' => $user])
			->setFrom(Yii::$app->getModule('user')->mailerOptions['from'])
			->setTo($user->email)
	...
	public function checkConfirmationToken($token)
	{
		...
		if ( $user )
		{
			...
			$roles = (array)Yii::$app->getModule('user')->rolesAfterRegistration;
